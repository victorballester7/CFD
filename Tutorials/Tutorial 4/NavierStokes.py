...
def LUdecomposition(LAP):
    """
    returns the Incomplete LU decomposition
...
# Matrices for advection
ua = np.zeros((NX, NY))
va = np.zeros((NX, NY))

# ustar and vstar matrices
...
# Building matrices and LU decomposition
...
for niter in range(nitermax):
    ...
    # Ghost points update
    VelocityGhostPoints(u_a, v_a)

    # Diffusion step
    ustar = u_a + dt * Laplacian(u) / Re
    vstar = v_a + dt * Laplacian(v) / Re
