...
def grad(x):
    ...
    """
    rst = np.empty((NX, NY, 2))

    rst[1:-1, 1:-1, 0] = (x[2:, 1:-1] - x[:-2, 1:-1]) / dx / 2
    rst[1:-1, 1:-1, 1] = (x[1:-1, 2:] - x[1:-1, :-2]) / dy / 2
...
def VelocityGhostPoints(u, v):
    ...
    """
    # bottom
    u[:, 0] = -u[:, 1]  # the point 0 is outside the domain and 1 is strictly inside
    v[:, 0] = -v[:, 1]
    # top
    u[:, -1] = 2 - u[:, -2]  # linear interpolation
    v[:, -1] = -v[:, -2]
    # right
    u[0, :] = -u[1, :]
    v[0, :] = -v[1, :]
...
#####################
tStart = t
Lap_matrix = BuildLAPN()
LUPN = LUdecomposition(Lap_matrix)
...
for niter in range(nitermax):
    ...
    phi[1:-1, 1:-1] = Resolve(LUPN, divstar[1:-1, 1:-1])
