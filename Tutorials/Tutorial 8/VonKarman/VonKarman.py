...
def Forme():
    ...
    DD = np.sqrt((xx - xx[ctry, ctrx])**2 + (yy - yy[ctry, ctrx])**2)

    # Obstacle spherique immobile
    Fe[1:-1, 1:-1] = np.where(DD < 0.15, 0, 1).transpose()
...
#    divstar = divstar - np.mean(divstar[1:-1,1:-1])
    ...
    if ((niter + 1) % modulo == 0):
        ...
        plt.clf()
        plt.title(plotlabel)
#        plt.quiver(xx[::4,::4],yy[::4,::4],np.transpose(u[1:-1:4,1:-1:4]),np.transpose(v[1:-1:4,1:-1:4]),0.9)
        vort = vorticite(u, v) * Fe
        plt.pcolor(xx, yy, np.transpose(vort[1:-1, 1:-1]))
        plt.clim(-30, 30)
        plt.axis('image')
        plt.draw()
        plt.pause(0.001)
